<template>
  <div class="space-y-1">
    ...
    >
      ...
      >
        <Icon
          :name="reversed ? 'lucide:arrow-up-narrow-wide' : 'lucide:arrow-down-wide-narrow'"
          size="14"
        />
    ...
    <p
      v-if="query"
      class="pb-1 text-xs text-text-tertiary"
    >
      Showing {{ displayedChapters.length }} of {{ chapters.length }} chapters
    </p>

    <div
      :class="[maxHeightClass, 'space-y-0.5 overflow-y-auto rounded-xl bg-secondary p-1']"
    >
      <div
      ...
      <NuxtLink
        ...
        :class="isActiveChapter(chapter.href) ? 'bg-accent/10 text-accent hover:bg-accent/15' : 'hover:bg-surface-hover'"
        @click="emit('select', chapter.href)"
      >
        <span
          class="line-clamp-1 flex-1 text-sm"
          :class="isActiveChapter(chapter.href) ? 'font-semibold text-accent' : 'text-text-primary'"
        >
          {{ chapter.title }}
        </span>
        <span
          ...
          :class="isActiveChapter(chapter.href) ? 'font-semibold text-accent' : 'text-text-tertiary'"
        >
...
const displayedChapters = computed(() => {
  ...
  const chapters = reversed.value ? [...props.chapters].reverse() : props.chapters;
...
const normalizedCurrentHref = computed(() => normalizeSourceHref(props.currentHref));
